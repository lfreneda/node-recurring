...
exports = module.exports = function (config) {
  ...
  if (config.interval === 'd' && config.options.skipWeekend) {
    let weekdayIndex = 0;

    while (
      (!config.limit || results.length < config.limit) &&
      (!config.iterations || iterations < config.iterations) &&
      (!config.end || occurrence.isSameOrBefore(config.end))
    ) {
      const day = occurrence.weekday()
      if (day !== 0 && day !== 6) {
        if (weekdayIndex % config.every === 0) {
          results.push(occurrence.format(config.options.format));
          iterations++;
